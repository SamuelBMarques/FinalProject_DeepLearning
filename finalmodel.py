...
def main():
    ...
    # Train stride widened from 250 (1s) to 500 (2s): the old stride gave
    # 96.7% overlap between consecutive windows, so the model was seeing
    # near-duplicate samples and could memorize them instead of
    # generalizing. 2s stride still gives plenty of windows but cuts
    # redundancy roughly in half.
    TRAIN_OFFSET = 500
    VAL_OFFSET   = 500

    # Test stride = full window length -> truly non-overlapping windows.
    # The previous value (2500) still overlapped by 66%, which can
    # inflate the reported test accuracy since neighboring windows share
    # most of their samples. Non-overlapping windows give an honest,
    # independent-sample estimate of generalization.
    ...
    NUM_EPOCHS  = 40
    LR          = 1e-3
    WEIGHT_DECAY = 5e-4        # raised from 1e-4: stronger L2 penalty
