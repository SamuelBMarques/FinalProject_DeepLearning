...
class SWDataset(Dataset):
    ...
    def __init__(self, folder_path, sampling_points, offset, tasks, subjects, freq=100):
        ...
        # CORREÇÃO: Converter o caminho para um objeto Path nativo do sistema operativo (corrige as barras no Windows)
        ...
        for subject in subjects:
            for task in tasks:
                ...
                # CORREÇÃO DE SEGURANÇA: Extrai dinamicamente o canal de sinal correto do pescoço
                if data.shape[1] > 2:
                ...
                # Converte o vetor para o formato de canais (1, time) exigido pela CNN
...
def main():
    FOLDER_PATH = "physionet.org/files/respiratory-oximetry-apnoea/1.0.0/Neck_Pulse_Oximeter_Data"  # root folder of the dataset
    ...
    # Model (in_channels=1 porque recebe 1 canal de sinal PPG)
    model = CNN1D(in_channels=1, sampling_points=SAMPLING_POINTS, num_classes=3).to(device)
    ...
    plt.tight_layout()
    plt.savefig("training_curves_neck_pulse.png")
    plt.show()
